...
function renderGrid() {
  ...
  for (var i = 0; i < themes.length; i++) {
    var t = themes[i];
    var swHtml = t.swatches.map(function(c){ return '<div class="tp-swatch" style="background:' + c + '"></div>'; }).join('');
    html += '<div class="tp-card' + (i===selected?' active':'') + '" data-i="' + i + '">'
      + buildMiniMock(t)
      + '<div class="tp-info">'
  ...
  for (var j = 0; j < cards.length; j++) {
    cards[j].addEventListener('click', (function(idx) {
      return function() { selected = idx; render(); };
    })(j));
  }
}
...
function renderSidebarBtn() {
  var t = themes[selected];
  var slugMap = {amber:'amber-classic',deepblue:'deep-blue',darkgreen:'dark-green',terracotta:'deep-maroon',olive:'burnt-orange',indigo:'midnight-blue',coral:'coral-red',peach:'peach',salmon:'salmon', earthkhaki: 'earth-khaki',ivory:'slate-ivory',spring:'spring'};
  var slug = slugMap[t.id] || t.id;
  var url = 'https://m-strux.matsusanjpn.com/p/color-' + slug + '.html';
  // サイドバーのボタン（theme.xml側）
...
if (applyBtn) {
  applyBtn.addEventListener('click', function() {
    var t = themes[selected];
    // テーマIDをハイフン区切りに変換: deepblue → deep-blue, darkgreen → dark-green 等
    var slug = t.id.replace(/([a-z])([A-Z])/g, '$1-$2').toLowerCase();
    // idが既にハイフンなしの場合はIDマップで対応
    var slugMap = {
      amber:      'amber-classic',
      deepblue:   'deep-blue',
      darkgreen:  'dark-green',
      terracotta: 'deep-maroon',
      olive:      'burnt-orange',
      indigo:     'midnight-blue',
      coral:      'coral-red',
      peach:      'peach',
      salmon:     'salmon',
      earthkhaki: 'earth-khaki',
      ivory:	  'slate-ivory',
      spring:	  'spring'
      
    };
    slug = slugMap[t.id] || slug;
...
// ===== 初期描画 =====
// SPA対応：DOMContentLoadedを使わず、要素があれば即描画
function init() {
  var grid = document.getElementById('tp-grid');
  if (!grid) {
    // まだDOMが準備できていない場合はMutationObserverで待機
...
function bindEvents() {
  ...
  var cb = document.getElementById('tp-copy-btn');
  if (cb && !cb._bound) {
    cb._bound = true;
  ...
  var ab = document.getElementById('tp-apply-btn');
  if (ab && !ab._bound) {
    ab._bound = true;
    ab.addEventListener('click', function() {
      var t = themes[selected];
      var slugMap = {amber:'amber-classic',deepblue:'deep-blue',darkgreen:'dark-green',terracotta:'deep-maroon',olive:'burnt-orange',indigo:'midnight-blue',coral:'coral-red',peach:'peach',salmon:'salmon'};
      var slug = slugMap[t.id] || t.id;
      location.href = 'https://m-strux.matsusanjpn.com/p/color-' + slug + '.html';
    });
  }
